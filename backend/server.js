...
const { detect } = require("tesseract.js");
...
app.post("/upload", upload.array("files", 10), async (req, res) => {
  try {
    ...
    if (files.length === 0) {
      return res.status(400).json({ error: "[x] No files provided!" });
    }
    ...
    for (const file of files) {
      try {
        ...
        console.log(
          `[✓] Created ${chunks.length} chunks for ${file.originalname}!`
        );
        ...
        console.log(`[✓] ${file.originalname} processed succesfully!`);
      } catch (error) {
    ...
    console.log(
      `[✓] Batch complete. Total documents: ${uploadedDocuments.length}, Total chunks: ${totalChunks}`
    );
...
app.post("/clear-all", async (req, res) => {
  try {
    ...
    uploadedDocuments = []; // Reset memory
    ...
    res
      .status(500)
      .json({ error: "Failed to clear documents", details: error.message });
  }
});
